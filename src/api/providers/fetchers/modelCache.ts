...
import { getVertexGatewayModels } from "./vertex-gateway"
...
const AUTH_SCOPED_PROVIDERS: ReadonlySet<RouterName> = new Set(["vertex-gateway"])
...
async function fetchModelsFromProvider(options: GetModelsOptions): Promise<ModelRecord> {
	...
	switch (provider) {
		...
		case "deepseek":
			...
			break
		case "vertex-gateway":
			models = await getVertexGatewayModels({ vertexSessionToken: options.apiKey, vertexGatewayBaseUrl: options.baseUrl })
			break
		default: {
...
export const refreshModels = async (options: GetModelsOptions): Promise<ModelRecord> => {
	...
	const refreshPromise = (async (): Promise<ModelRecord> => {
		try {
			...
			// For auth-scoped providers (vertex-gateway) we MUST NOT return cached models from a prior
