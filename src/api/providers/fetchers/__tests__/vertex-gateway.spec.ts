// npx vitest run src/api/providers/fetchers/__tests__/vertex-gateway.spec.ts

import axios from "axios"

import { getVertexGatewayModels, parseVertexGatewayModel } from "../vertex-gateway"

vitest.mock("axios")
vitest.mock("../../../../services/vertex-auth", () => ({
	getCachedVertexToken: vitest.fn(() => ""),
	getVertexBaseUrl: vitest.fn(() => "https://example.test"),
	resolveVertexGatewaySessionToken: vitest.fn((profileToken?: string) => profileToken || undefined),
}))
const mockedAxios = axios as any

describe("Vertex Gateway Fetchers", () => {
	beforeEach(() => {
		vitest.clearAllMocks()
	})

	describe("getVertexGatewayModels", () => {
		const baseUrl = "https://example.test/api/gateway/v1"
		const token = "vertex_ext_test_token"
		...
		it("forwards the bearer token and timeout, filters non-language models", async () => {
			...
			const models = await getVertexGatewayModels({
				vertexGatewayBaseUrl: baseUrl,
		...
		it("omits the Authorization header when no token is provided", async () => {
			...
			await getVertexGatewayModels({ vertexGatewayBaseUrl: baseUrl } as any)
	...
	describe("parseVertexGatewayModel", () => {
		it("delegates to the vercel-ai-gateway parser", () => {
			const result = parseVertexGatewayModel({
				id: "anthropic/claude-sonnet-4",
				model: {
					id: "anthropic/claude-sonnet-4",
					object: "model",
