...
describe("UnboundHandler", () => {
	...
	it("identifies itself as Vertex in the Unbound request headers", () => {
		new UnboundHandler({
		...
		expect(OpenAI).toHaveBeenCalledWith(
			expect.objectContaining({
				defaultHeaders: expect.objectContaining({
					"X-Unbound-Metadata": JSON.stringify({ labels: [{ key: "app", value: "vertex" }] }),
				}),
			}),
		)
	})

	it("identifies itself as Vertex in per-request Unbound metadata", async () => {
