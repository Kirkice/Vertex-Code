 ...
 *
 * Provides proxy configuration for all outbound HTTP/HTTPS requests from the Vertex Code extension.
...
async function configureUndiciProxy(config: ProxyConfig): Promise<void> {
	...
	try {
		...
		// To ensure Vertex Code's `fetch()` calls are proxied, patch global fetch in debug mode.
