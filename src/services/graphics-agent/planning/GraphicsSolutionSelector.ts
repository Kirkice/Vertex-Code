import type {
  ...
  GraphicsSolutionOverride,
  GraphicsSolutionRecommendation,
  GraphicsSolutionRulePackage,
} from "@roo-code/types";

interface GraphicsSolutionSelectorOptions {
  now?: () => Date;
  rulePackages?: readonly GraphicsSolutionRulePackage[];
  override?: GraphicsSolutionOverride;
...
const candidateDefinitions: readonly CandidateDefinition[] = [
  {
    level: "configuration",
    label: "Existing parameters or asset configuration",
    baseScore: 16,
    complexity: 1,
  },
  {
    level: "shader",
    label: "Material or shader implementation",
    baseScore: 14,
    complexity: 2,
  },
  {
  ...
  {
    level: "post-process",
    label: "Post-processing pipeline extension",
    baseScore: 9,
    complexity: 4,
  },
  {
    level: "render-graph",
    label: "Render Graph or render-pipeline modification",
    baseScore: 3,
    complexity: 7,
  },
  {
    level: "compute",
    label: "Compute shader or asynchronous GPU work",
    baseScore: 4,
    complexity: 6,
  },
  {
    level: "cpu-client",
    label: "CPU or client-side implementation",
    baseScore: 6,
    complexity: 3,
  },
];
...
const scoringRules: readonly ScoringRule[] = [
  {
    ...
    pattern: bilingualPattern(
      "parameter|material|texture|asset|prefab|configuration",
      "配置|参数|材质|贴图|资源",
    ),
    score: 12,
    reason:
      "The brief can potentially reuse existing material, asset, or configuration controls.",
  },
  {
    levels: ["shader"],
    pattern: bilingualPattern(
    ...
    score: 18,
    reason:
      "The requested visual behavior is primarily local to a surface or material.",
  },
  {
    ...
    score: 15,
    reason:
      "The effect appears to require camera-wide data or a dedicated rendering stage.",
  ...
  },
  {
    ...
    score: 20,
    reason:
      "The brief explicitly describes a screen-space post-processing effect.",
  },
  {
    levels: ["render-graph"],
    pattern: bilingualPattern(
    ...
    score: 22,
    reason:
      "The requirement may need new cross-pass resources or a rendering-pipeline change.",
  ...
  },
  {
    ...
    score: 13,
    reason:
      "The feature requires meaningful gameplay, event, or lifecycle orchestration.",
  },
];

const highRiskLevels = new Set<GraphicsSolutionLevel>([
  "render-graph",
  "compute",
]);

const defaultCapabilityRequirements: Partial<Record<GraphicsSolutionLevel, string[]>> = {
  "renderer-pass": ["renderer-pass"],
  "post-process": ["post-process"],
  "render-graph": ["render-graph"],
  compute: ["compute-shader"],
};

const defaultResourceNeeds: Partial<Record<GraphicsSolutionLevel, string[]>> = {
  shader: ["material-or-shader"],
  "renderer-pass": ["camera-color-or-depth-target"],
  "render-graph": ["cross-pass-resource"],
  compute: ["structured-buffer"],
};

const defaultEstimatedCosts: Partial<Record<GraphicsSolutionLevel, number>> = {
  configuration: 1,
  shader: 2,
  "renderer-pass": 4,
  "post-process": 5,
...
function createCandidates(): Map<
  GraphicsSolutionLevel,
  GraphicsSolutionCandidate & { complexity: number }
> {
  return new Map(
    candidateDefinitions.map((definition) => [
      definition.level,
      {
        ...
        risks: [],
        rejectionReasons: [],
        requiredCapabilities: defaultCapabilityRequirements[definition.level] ?? [],
        resourceNeeds: defaultResourceNeeds[definition.level] ?? [],
        estimatedCost: defaultEstimatedCosts[definition.level] ?? definition.complexity,
      },
    ]),
  );
}
...
function addProjectEvidence(
  ...
  const projectText = [
    ...
    ...profile.architectureIndex.findings.map(
      (finding) => `${finding.kind} ${finding.detail}`,
    ),
  ].join("\n");
  ...
  const evidenceRules: readonly [GraphicsSolutionLevel, RegExp, string][] = [
    [
      "shader",
      /shader|lightmode|keyword|include/i,
      "The project already contains reusable shader entry points.",
    ],
    [
      "renderer-pass",
...
function finalizeCandidates(
  ...
  const ranked = [...candidates.values()].sort(
    (left, right) =>
      right.score - left.score || left.complexity - right.complexity,
  );
  ...
  return ranked.map(({ complexity: _complexity, ...candidate }, index) => {
    ...
    candidate.confidence =
      index === 0 && candidate.reasons.length >= 2
        ? "high"
        : scoreGap <= 8
          ? "medium"
          : "low";
...
export function selectGraphicsSolution(
  ...
  const decisionHistory: NonNullable<
    GraphicsSolutionRecommendation["decisionHistory"]
  > = [];
  ...
  for (const rulePackage of options.rulePackages ?? []) {
    for (const rule of rulePackage.rules) {
      const pattern = new RegExp(rule.pattern, "iu");
      if (!pattern.test(briefText)) continue;
      for (const level of rule.levels) {
        const candidate = candidates.get(level);
        if (!candidate) continue;
        candidate.score += rule.score;
        ...
        candidate.requiredCapabilities = [
          ...new Set([...(candidate.requiredCapabilities ?? []), ...(rule.requiredCapabilities ?? [])]),
        ];
        candidate.resourceNeeds = [
          ...new Set([...(candidate.resourceNeeds ?? []), ...(rule.resourceNeeds ?? [])]),
        ];
        if (rule.estimatedCost !== undefined) candidate.estimatedCost = rule.estimatedCost;
      }
      decisionHistory.push({
        source: `rule-package:${rulePackage.id}`,
        decision: rule.levels.join(","),
        reason: rule.reason,
  ...
  const availableCapabilities = new Set((options.availableCapabilities ?? []).map((capability) => capability.toLowerCase()));
  for (const candidate of candidates.values()) {
    const missingCapabilities = (candidate.requiredCapabilities ?? []).filter(
      (capability) => availableCapabilities.size > 0 && !availableCapabilities.has(capability.toLowerCase()),
    );
    if (missingCapabilities.length > 0) {
      candidate.rejectionReasons.push(`Missing required capabilities: ${missingCapabilities.join(", ")}.`);
      candidate.score = Math.max(0, candidate.score - 100);
    }
    if (options.maxCost !== undefined && (candidate.estimatedCost ?? 0) > options.maxCost) {
      candidate.rejectionReasons.push(`Estimated cost exceeds the budget of ${options.maxCost}${options.costUnit ? ` ${options.costUnit}` : ""}.`);
      candidate.score = Math.max(0, candidate.score - 100);
    }
  }
  const rankedCandidates = finalizeCandidates(candidates);
  const automaticRecommendation = rankedCandidates[0];
  const recommended = options.override
    ? (rankedCandidates.find(
        (candidate) => candidate.level === options.override?.level,
      ) ?? automaticRecommendation)
    : automaticRecommendation;
  if (options.override && recommended.level === options.override.level) {
    decisionHistory.push({
      source: "human-override",
      decision: options.override.level,
  ...
  if (!brief.performanceBudget.trim())
    assumptions.push("No explicit performance budget was provided.");
  if (!brief.targetPlatforms.trim())
    assumptions.push("No target platform or graphics API was provided.");
  ...
  return {
    ...
    assumptions,
    constraints: {
      availableCapabilities: options.availableCapabilities ? [...options.availableCapabilities] : undefined,
      maxCost: options.maxCost,
      costUnit: options.costUnit,
    },
    decisionHistory: decisionHistory.length > 0 ? decisionHistory : undefined,
