...
export class BrowserService {
	 ...
	 * Wait for selector
	 */
	async waitForSelector(options: WaitForSelectorOptions): Promise<BrowserActionResult> {
		if (!this.page) {
			return { success: false, error: "Browser not launched" }
		}
	 ...
	 */
	async scroll(options: ScrollOptions): Promise<BrowserActionResult> {
		if (!this.page) {
			return { success: false, error: "Browser not launched" }
		}
		...
		try {
			if (options.selector) {
				...
				await this.page.evaluate(
					(args: { sel: string; scrollX: number; scrollY: number }) => {
				...
				const scrollY = options.y || 0
				await this.page.evaluate(
					(args: { scrollX: number; scrollY: number }) => {
