...
describe("newTaskTool", () => {
	...
	describe("VSCode setting: newTaskRequireTodos", () => {
		...
		it("should check VSCode setting with Package.name configuration key", async () => {
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("vertex")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})

		it("should use current Package.name value (vertex-nightly) when accessing VSCode configuration", async () => {
			...
			;(pkg.Package as any).name = "vertex-nightly"
			...
			try {
				...
				expect(mockGetConfiguration).toHaveBeenCalledWith("vertex-nightly")
				expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
			} finally {
