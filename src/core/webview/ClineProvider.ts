	...
	implements vscode.WebviewViewProvider, TaskProviderLike
{
	...
	*/
	...
	async resolveWebviewView(webviewView: vscode.WebviewView | vscode.WebviewPanel) {
		...
		// Ensure vertex-gateway profile is seeded for users who signed in before this feature existed.
		// Without this, users with a valid cached token but no vertex-gateway profile would need to
		// re-authenticate to use Vertex Gateway. Fire-and-forget to avoid blocking webview init.
	...
	/**
	 * Seeds the vertex-gateway provider profile for users who have a cached auth token
	 * but no profile (e.g., users who signed in before Vertex Gateway was added), or
	 * who have an empty/imported profile without a token.
	 * Called once per webview init; handleVertexCallback is idempotent so repeated calls are safe.
	 */
	private async ensureVertexGatewayProfileSeeded(): Promise<void> {
		const { getCachedVertexToken, getVertexBaseUrl } = await import("../../services/vertex-auth")
		const token = getCachedVertexToken()
		if (!token) return
		const expectedGatewayBaseUrl = `${getVertexBaseUrl()}/api/gateway/v1`

		// Check ALL vertex-gateway profiles — only skip seeding if every profile has the current token.
		// Using .find() would miss stale tokens in duplicate/renamed profiles since handleVertexCallback
		...
		const vertexGatewayProfiles = allProfiles.filter((p) => p.apiProvider === "vertex-gateway")

		if (vertexGatewayProfiles.length === 0) {
			this.log("[ensureVertexGatewayProfileSeeded] No vertex-gateway profile found, creating one")
		} else {
			let allUpToDate = true

			for (const entry of vertexGatewayProfiles) {
				try {
					const fullProfile = await this.providerSettingsManager.getProfile({ name: entry.name })
					if (
						fullProfile.vertexSessionToken !== token ||
					...
					allUpToDate = false
					this.log("[ensureVertexGatewayProfileSeeded] Failed to read existing profile, will re-seed")
					break
				}
			}
		...
		await this.handleVertexCallback(token)
	}
	...
	async handleVertexCallback(token: string) {
		...
		// Save the vertex-gateway provider profile with the session token so that
		// VertexGatewayHandler can authenticate without any manual user input.
		//
		// activate: true ONLY if Vertex Gateway is already the active profile — this pushes
		...
		// must explicitly select Vertex Gateway in settings if they want to use it.
		try {
			const { apiConfiguration } = await this.getState()
			...
			// Derive the gateway base URL from VERTEX_BASE_URL so that non-prod environments
			...
			const { getVertexBaseUrl } = await import("../../services/vertex-auth")
			const derivedGatewayBaseUrl = `${getVertexBaseUrl()}/api/gateway/v1`

			// Check if Vertex Gateway is the currently active profile by apiProvider identity,
			// not by profile name (profile names are user-renameable).
			const isVertexGatewayActive = currentSettings.apiProvider === "vertex-gateway"

			// Always scan ALL profiles and update every vertex-gateway profile with the new token.
			...
			// first vertex-gateway profile it finds — if that profile has a stale token, requests fail.
			const allProfiles = await this.providerSettingsManager.listConfig()
			const vertexProfiles = allProfiles.filter((p) => p.apiProvider === "vertex-gateway")

			if (vertexProfiles.length === 0) {
				// No existing vertex-gateway profile — create the canonical default.
				const newConfiguration: ProviderSettings = {
					apiProvider: "vertex-gateway",
					vertexSessionToken: token,
					vertexGatewayModelId: apiConfiguration.vertexGatewayModelId,
					vertexGatewayBaseUrl: derivedGatewayBaseUrl,
				}
				// Activate only if vertex-gateway was the active provider (shouldn't happen if
				// no profiles exist, but defensive).
				await this.upsertProviderProfile("Vertex Gateway", newConfiguration, isVertexGatewayActive)
			} else {
				// Update every existing vertex-gateway profile with the new token and the
				// derived base URL so that environment-specific routing stays consistent.
				for (const entry of vertexProfiles) {
					const isActiveProfile = isVertexGatewayActive && entry.name === currentApiConfigName
			...
			this.log(
				`[handleVertexCallback] Failed to save vertex-gateway profile: ${
					error instanceof Error ? error.message : String(error)
				}`,
			)
		}
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		let vertexState: {
			...
			vertexUserName: string | undefined
			vertexUserEmail: string | undefined
			vertexUserImage: string | undefined
			vertexBaseUrl: string
			deviceName: string
		} = {
			vertexIsAuthenticated: false,
			vertexUserName: undefined,
			vertexUserEmail: undefined,
			vertexUserImage: undefined,
			vertexBaseUrl: "https://www.vertex.dev",
			deviceName: os.hostname(),
		}

		try {
			const { isVertexAuthenticated, getCachedVertexUserInfo, getVertexBaseUrl } = await import(
				"../../services/vertex-auth"
			)
			const userInfo = getCachedVertexUserInfo()
			vertexState = {
				vertexIsAuthenticated: await isVertexAuthenticated(),
				vertexUserName: userInfo.name,
				vertexUserEmail: userInfo.email,
				vertexUserImage: userInfo.image,
				vertexBaseUrl: getVertexBaseUrl(),
				deviceName: os.hostname(),
			}
		} catch {
			// Keep the default unauthenticated state if the optional Vertex auth service is unavailable.
		}
	 ...
	 * @returns true always (MDM service removed)
	 */
	...
	public async getTelemetryProperties(): Promise<Record<string, unknown>> {
		return {
