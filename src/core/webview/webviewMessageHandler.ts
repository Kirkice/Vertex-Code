...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		case "vertexSignOut": {
			try {
				const { disconnectVertex } = await import("../../services/vertex-auth")
				await disconnectVertex()

				// Clear vertexSessionToken from ALL provider profiles with apiProvider === "vertex-gateway".
				// Profiles are user-renameable, so we cannot rely on a hardcoded name like "Vertex Gateway".
				// We must scan all profiles and clear tokens from any that use the vertex-gateway provider.
				try {
					...
					for (const entry of allProfiles) {
						if (entry.apiProvider !== "vertex-gateway") {
							continue
						}
						...
						try {
							...
							const { vertexSessionToken: _removed, ...cleanedProfile } = profile
							...
							const isThisProfileActive = isVertexGatewayActive && currentApiConfigName === entry.name

							if (isThisProfileActive) {
								await provider.upsertProviderProfile(entry.name, cleanedProfile, true)
								provider.log(
									`[vertexSignOut] Cleared vertexSessionToken from "${entry.name}" profile and updated in-memory handler`,
								)
							} else if (profile.vertexSessionToken) {
								await provider.providerSettingsManager.saveConfig(entry.name, cleanedProfile)
								provider.log(`[vertexSignOut] Cleared vertexSessionToken from "${entry.name}" profile`)
							}
						} catch (profileError) {
							// Log but continue to the next profile so one failure doesn't
							// leave other profiles holding a stale token.
							provider.log(
								`[vertexSignOut] Failed to clear profile token for "${entry.name}": ${profileError instanceof Error ? profileError.message : String(profileError)}`,
							)
						}
					}
				} catch (profileError) {
					...
					provider.log(
						`[vertexSignOut] Failed to list profiles for token cleanup: ${profileError instanceof Error ? profileError.message : String(profileError)}`,
					)
				}
