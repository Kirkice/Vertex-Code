...
import type { WorktreeHostPort } from "../ports"
...
export async function handleListWorktrees(provider: WorktreeHostPort): Promise<WorktreeListResponse> {
...
export async function handleSwitchWorktree(
	...
	try {
		...
		if (newWindow) {
			// Set the auto-open path so the new window opens Vertex Code sidebar.
...
export async function handleGetAvailableBranches(provider: WorktreeHostPort): Promise<BranchInfo> {
	const cwd = provider.cwd
...
export async function handleGetWorktreeDefaults(provider: WorktreeHostPort): Promise<WorktreeDefaultsResponse> {
...
export async function handleGetWorktreeIncludeStatus(provider: WorktreeHostPort): Promise<WorktreeIncludeStatus> {
	const cwd = provider.cwd
	return worktreeIncludeService.getStatus(cwd)
}

export async function handleCheckBranchWorktreeInclude(provider: WorktreeHostPort, branch: string): Promise<boolean> {
	const cwd = provider.cwd
...
export async function handleCheckoutBranch(provider: WorktreeHostPort, branch: string): Promise<WorktreeResult> {
	const cwd = provider.cwd
