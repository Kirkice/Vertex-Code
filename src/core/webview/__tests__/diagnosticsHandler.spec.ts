...
describe("generateErrorDiagnostics", () => {
	...
	it("generates a diagnostics file with error metadata and history", async () => {
		...
		expect(result.filePath).toContain("vertex-diagnostics-")
		...
		expect(String(writtenPath)).toContain("vertex-diagnostics-test-tas")
		expect(String(writtenContent)).toContain(
			"// Please share this file with Vertex Support (support@vertex.dev) to diagnose the issue faster",
		)
