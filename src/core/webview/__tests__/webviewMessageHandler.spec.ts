...
vi.mock("../../../services/vertex-auth", () => ({
	disconnectVertex: vi.fn().mockResolvedValue(undefined),
}))
...
describe("webviewMessageHandler - requestRouterModels", () => {
	...
	it("handles individual provider failures gracefully", async () => {
		...
		mockGetModels
			...
			.mockResolvedValueOnce(mockModels) // vertex-gateway
	...
	it("handles Error objects and string errors correctly", async () => {
		...
		mockGetModels
			...
			.mockRejectedValueOnce(new Error("Vertex Gateway error")) // vertex-gateway
			.mockRejectedValueOnce(new Error("LiteLLM connection failed")) // litellm
	...
	it("returns an explicit removal error for requestRooModels", async () => {
		...
		expect(mockClineProvider.postMessageToWebview).toHaveBeenCalledWith({
			type: "singleRouterModelFetchResponse",
			success: false,
			error: "Vertex Code Router has been removed. Please select and configure a different provider.",
			values: { provider: "roo" },
		})
	})
...
describe("vertexSignOut", () => {
	beforeEach(() => {
		vi.clearAllMocks()
	})

	it("disconnects Vertex and clears tokens from all vertex-gateway profiles", async () => {
		...
		;(mockClineProvider as any).contextProxy = {
			...
			getProviderSettings: vi.fn().mockReturnValue({ apiProvider: "vertex-gateway" }),
			getValues: vi.fn().mockReturnValue({ currentApiConfigName: "Vertex Gateway" }),
		}
		;(mockClineProvider as any).providerSettingsManager = {
			...
			getProfile: vi
				...
				.mockResolvedValueOnce({
					...
					vertexSessionToken: "token-active",
					vertexGatewayModelId: "anthropic/claude-sonnet-4",
				})
				.mockResolvedValueOnce({
					apiProvider: "vertex-gateway",
					vertexSessionToken: "token-backup",
				}),
			saveConfig,
		}
		;(mockClineProvider as any).upsertProviderProfile = upsertProviderProfile

		await webviewMessageHandler(mockClineProvider, { type: "vertexSignOut" })

		expect(disconnectVertex).toHaveBeenCalled()
		expect(upsertProviderProfile).toHaveBeenCalledWith(
			"Vertex Gateway",
			expect.not.objectContaining({ vertexSessionToken: expect.anything() }),
			true,
		)
