...
vi.mock("../../../services/vertex-auth", () => ({
	getVertexBaseUrl: vi.fn(() => "https://www.vertex.dev"),
	getCachedVertexToken: vi.fn(),
	handleAuthCallback: vi.fn(),
	setVertexUserInfo: vi.fn(),
	disconnectVertex: vi.fn(),
}))
...
describe("ClineProvider - Router Models", () => {
	...
	test("handles requestRouterModels with individual provider failures", async () => {
		...
		vi.mocked(getModels)
			...
			.mockResolvedValueOnce(mockModels) // vertex-gateway success
...
describe("ClineProvider - Comprehensive Edit/Delete Edge Cases", () => {
	...
	describe("Vertex auth profile sync", () => {
		beforeEach(async () => {
			const { getCachedVertexToken } = await import("../../../services/vertex-auth")
			vi.mocked(getCachedVertexToken).mockReturnValue("")
		})

		describe("handleVertexCallback", () => {
			it("creates a Vertex Gateway profile when none exists", async () => {
				vi.spyOn(provider, "getState").mockResolvedValue({
					apiConfiguration: { vertexGatewayModelId: "anthropic/claude-sonnet-4" },
				} as any)
				...
				await provider.handleVertexCallback("vertex_ext_token")

				expect(upsertSpy).toHaveBeenCalledWith(
					"Vertex Gateway",
					expect.objectContaining({
						apiProvider: "vertex-gateway",
						vertexSessionToken: "vertex_ext_token",
						vertexGatewayBaseUrl: "https://www.vertex.dev/api/gateway/v1",
					}),
					false,
				)
			})

			it("updates every vertex-gateway profile and activates only the active one", async () => {
				vi.spyOn(provider, "getState").mockResolvedValue({
					apiConfiguration: { vertexGatewayModelId: "anthropic/claude-sonnet-4" },
				} as any)
				vi.spyOn(provider.contextProxy, "getProviderSettings").mockReturnValue({
				...
				vi.spyOn(provider.contextProxy, "getValues").mockReturnValue({
					currentApiConfigName: "Vertex Gateway",
				} as any)
				const upsertSpy = vi.spyOn(provider, "upsertProviderProfile").mockResolvedValue("profile-id")
				...
				;(provider as any).providerSettingsManager = {
					listConfig: vi.fn().mockResolvedValue([
						{ name: "Vertex Gateway", apiProvider: "vertex-gateway" },
						{ name: "Backup Vertex", apiProvider: "vertex-gateway" },
					]),
					getProfile: vi
						.fn()
						.mockResolvedValueOnce({
							apiProvider: "vertex-gateway",
							vertexSessionToken: "old-token",
							vertexGatewayBaseUrl: "https://old.example/api/gateway/v1",
						})
						.mockResolvedValueOnce({
							apiProvider: "vertex-gateway",
							vertexSessionToken: "old-token",
				...
				await provider.handleVertexCallback("new-token")

				expect(upsertSpy).toHaveBeenCalledWith(
					"Vertex Gateway",
					expect.objectContaining({
						vertexSessionToken: "new-token",
						vertexGatewayBaseUrl: "https://www.vertex.dev/api/gateway/v1",
					}),
					true,
				)
			...
			it("logs and posts state when profile persistence fails", async () => {
				...
				expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(
					expect.stringContaining("[handleVertexCallback] Failed to save vertex-gateway profile"),
				)
		...
		describe("ensureVertexGatewayProfileSeeded", () => {
			it("does nothing when no cached auth token exists", async () => {
				const handleSpy = vi.spyOn(provider, "handleVertexCallback").mockResolvedValue(undefined)

				;(provider as any).providerSettingsManager = {
					listConfig: vi.fn(),
				}

				await (provider as any).ensureVertexGatewayProfileSeeded()

				expect(handleSpy).not.toHaveBeenCalled()
			})

			it("skips seeding when every vertex-gateway profile already has the current token and base URL", async () => {
				const { getCachedVertexToken } = await import("../../../services/vertex-auth")
				vi.mocked(getCachedVertexToken).mockReturnValue("current-token")
				const handleSpy = vi.spyOn(provider, "handleVertexCallback").mockResolvedValue(undefined)

				;(provider as any).providerSettingsManager = {
					listConfig: vi.fn().mockResolvedValue([{ name: "Vertex Gateway", apiProvider: "vertex-gateway" }]),
					getProfile: vi.fn().mockResolvedValue({
						vertexSessionToken: "current-token",
						vertexGatewayBaseUrl: "https://www.vertex.dev/api/gateway/v1",
					}),
				}
			...
			it("re-seeds when any vertex-gateway profile has a stale or missing token", async () => {
				const { getCachedVertexToken } = await import("../../../services/vertex-auth")
				vi.mocked(getCachedVertexToken).mockReturnValue("fresh-token")
				const handleSpy = vi.spyOn(provider, "handleVertexCallback").mockResolvedValue(undefined)

				;(provider as any).providerSettingsManager = {
					listConfig: vi.fn().mockResolvedValue([{ name: "Vertex Gateway", apiProvider: "vertex-gateway" }]),
					getProfile: vi.fn().mockResolvedValue({
						vertexSessionToken: "stale-token",
						vertexGatewayBaseUrl: "https://www.vertex.dev/api/gateway/v1",
					}),
				}
			...
			it("re-seeds when any vertex-gateway profile has a stale base URL", async () => {
				const { getCachedVertexToken } = await import("../../../services/vertex-auth")
				vi.mocked(getCachedVertexToken).mockReturnValue("current-token")
				const handleSpy = vi.spyOn(provider, "handleVertexCallback").mockResolvedValue(undefined)
