...
describe("registerCommands handlers", () => {
	...
	it("settingsButtonClicked posts both settingsButtonClicked and didBecomeVisible actions", () => {
		handlers["vertex.settingsButtonClicked"]()
	...
	it("historyButtonClicked posts historyButtonClicked action", () => {
		handlers["vertex.historyButtonClicked"]()
	...
	it("marketplaceButtonClicked posts marketplaceButtonClicked action", () => {
		handlers["vertex.marketplaceButtonClicked"]()
	...
	it("toggleAutoApprove awaits postMessage with toggleAutoApprove action", async () => {
		...
		const handlerPromise = handlers["vertex.toggleAutoApprove"]() as Promise<unknown>
		let settled = false
	...
	it("focusInput awaits postMessage on the registered provider when a sidebar panel is active", async () => {
		...
		const handlerPromise = handlers["vertex.focusInput"]() as Promise<unknown>
		let settled = false
		void handlerPromise.then(() => {
			settled = true
		})
	...
	it("focusInput does not post when no sidebar panel is active", async () => {
		await handlers["vertex.focusInput"]()
	...
	it.each([
		{ command: "vertex.settingsButtonClicked", prefix: "settingsButtonClicked", expectedCalls: 2 },
		{ command: "vertex.historyButtonClicked", prefix: "historyButtonClicked", expectedCalls: 1 },
		{ command: "vertex.marketplaceButtonClicked", prefix: "marketplaceButtonClicked", expectedCalls: 1 },
		{ command: "vertex.acceptInput", prefix: "acceptInput", expectedCalls: 1 },
	])(
	...
	it("toggleAutoApprove logs to outputChannel when postMessageToWebview rejects", async () => {
		...
		await handlers["vertex.toggleAutoApprove"]()
