...
import { handleAuthCallback as handleVertexAuthCallback, setVertexUserInfo } from "../services/vertex-auth"

/**
 * Persist the Vertex session token to every active provider instance.
 *
 * The profile settings write (handleVertexCallback) must run on any active
 * instance — not just the visible one — so the vertex-gateway vertexSessionToken is
 * persisted even when the sidebar/panel is hidden at callback time.
 *
 * Run sequentially (NOT Promise.all): each ClineProvider's handleVertexCallback
 ...
 * the race.
 */
async function propagateVertexGatewayCallback(token: string): Promise<void> {
	const allInstances = ClineProvider.getAllInstances()
	for (const instance of allInstances) {
		try {
			await instance.handleVertexCallback(token)
		} catch (error) {
			console.error(
				"Failed to persist Vertex Gateway token for a provider instance:",
	...
	switch (path) {
		...
		case "/auth-callback": {
			...
			if (token) {
				...
				const success = await handleVertexAuthCallback(token)
				if (success) {
					// Store user info after successful auth validation (regardless of webview visibility)
					// Always call setVertexUserInfo to clear stale data when fields are missing
					await setVertexUserInfo({
						name,
						email,
						image,
					})
					await propagateVertexGatewayCallback(token)
				}
			}
			break
		}
