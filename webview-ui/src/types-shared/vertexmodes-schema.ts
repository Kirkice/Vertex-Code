/**
 * Builds the Zod schema for .vertexmodes configuration files and converts it
 * to JSON Schema (draft-07). This module is the single source of truth for
 * both the generator script (scripts/generate-vertexmodes-schema.ts) and the
...
// Build the top-level .vertexmodes schema.
const vertexmodesZodSchema = z
	.object({
...
/**
 * Generates the JSON Schema object for .vertexmodes configuration files.
 * Includes metadata fields ($id, title, description).
 */
export function generateRoomodesJsonSchema(): Record<string, unknown> {
	const jsonSchema = zodToJsonSchema(vertexmodesZodSchema, {
		$refStrategy: "none",
		target: "jsonSchema7",
	}) as Record<string, unknown>

	jsonSchema["$id"] = "https://github.com/Vertex-AI-Org/Vertex-AI/blob/main/schemas/vertexmodes.json"
	jsonSchema["title"] = "Roo Code Custom Modes"
	jsonSchema["description"] = "Schema for .vertexmodes configuration files used by Roo Code to define custom modes."
