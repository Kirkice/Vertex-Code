...
const createDots = ({ gridSize, cellSize, outerRadius }: DotFieldConfig): Dot[] => {
	...
	return Array.from({ length: gridSize * gridSize }, (_, index) => {
		...
		const normalized = distance / outerRadius
...
const DEFAULT_DOTS = createDots(DEFAULT_DOT_FIELD_CONFIG)

const LARGE_DOTS = createDots(LARGE_DOT_FIELD_CONFIG)
...
const RooHero = ({ size = 128 }: RooHeroProps) => {
	const isLargeLogo = size >= 256
	const dotFieldConfig = isLargeLogo ? LARGE_DOT_FIELD_CONFIG : DEFAULT_DOT_FIELD_CONFIG
	const dots = isLargeLogo ? LARGE_DOTS : DEFAULT_DOTS
