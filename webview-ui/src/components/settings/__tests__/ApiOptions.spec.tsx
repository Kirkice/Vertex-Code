...
import { type ModelInfo, type ProviderSettings, openAiModelInfoSaneDefaults } from "@vertex-code/types"
import { openAiCodexDefaultModelId, vertexGatewayDefaultModelId } from "@vertex-code/types"
...
describe("ApiOptions", () => {
	...
	it("initializes vertexGatewayModelId to its default when switching provider to vertex-gateway", () => {
		// Regression: vertex-gateway was previously missing from PROVIDER_MODEL_CONFIG, so switching
		// providers never seeded vertexGatewayModelId. Configs were left without a model id, which
		...
		const providerSelectContainer = screen.getByTestId("provider-select")
		const providerSelect = providerSelectContainer.querySelector("select") as HTMLSelectElement
		fireEvent.change(providerSelect, { target: { value: "vertex-gateway" } })

		expect(mockSetApiConfigurationField).toHaveBeenCalledWith("apiProvider", "vertex-gateway")
		expect(mockSetApiConfigurationField).toHaveBeenCalledWith("vertexGatewayModelId", vertexGatewayDefaultModelId, false)
	})
